...
CUSTOM_REMEDIATION_STEPS = """
You are a cybersecurity expert specializing in vulnerability remediation. Based on the remediation text, product, package, and CVE information provided below, generate a comprehensive, step-by-step remediation plan. The plan should be actionable, technically accurate, and suitable for system administrators.
...
1. **Identify Affected Systems** (Step 1)
   - Commands to check if the system has the vulnerable product/package
   - Verify CVE presence
   - Check installed package versions for {package}

2. **Locate Vulnerable Package** (Step 2)
   - Commands to find installed {package}
   - Version verification commands
   - Package manager specific queries for {product}

3. **Review CVE Details** (Step 3)
   - Commands to fetch CVE {cve} information
   - Check severity and CVSS score
   - Review vulnerability description

4. **Apply Security Updates** (Step 4)
   - Update commands for {package}
   - Follow the remediation text guidance
   - Version-specific upgrade paths for {product}

5. **Verify Patch Installation** (Step 5)
   - Commands to confirm {package} is updated
   - Verification that {cve} is resolved
   - Post-update validation checks

6. **Restart Affected Services** (Step 6)
   - Service restart commands for {product}
   - Daemon reload if needed
   - Service status verification

7. **Reboot System if Required** (Step 7)
   - Check if reboot is necessary for {product}
   - Reboot command
   - Post-reboot validation

8. **Apply Temporary Mitigations** (Step 8, if applicable)
   - Workarounds based on remediation text
   - Network restrictions
   - Access control measures
   - Mark as automation_possible: false

9. **Validate Remediation** (Step 9)
   - Re-scan commands for {cve}
   - Verification that vulnerability is no longer present
   - Final validation checks for {package}

RULES:
1. Generate commands appropriate for the product's operating system
2. Include specific CVE ID ({cve}) in commands where applicable
3. Reference the package name ({package}) in all relevant commands
4. Consider the product context ({product}) when generating commands
5. Follow the guidance provided in the remediation text
6. Provide realistic expected outputs for each command
7. Set automation_possible to true for commands that can be scripted, false for manual interventions
8. Ensure all commands are syntactically correct and safe to execute
9. Include error handling suggestions in descriptions
10. Respond ONLY with a valid JSON array, no additional text or markdown formatting
11. Ensure the JSON is properly escaped and formatted
